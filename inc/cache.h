...
struct cache_stats {
  ...
  //bank stats
  std::vector<std::array<std::array<uint64_t, NUM_CPUS>, champsim::to_underlying(access_type::NUM_TYPES)>> bank_hits;
  std::vector<std::array<std::array<uint64_t, NUM_CPUS>, champsim::to_underlying(access_type::NUM_TYPES)>> bank_misses;
  std::vector<double> bank_avg_miss_latency;
  std::vector<uint64_t> bank_total_miss_latency;

  //constructor for bank stats
    cache_stats() = default;
    explicit cache_stats(uint32_t num_banks)
        : bank_hits(num_banks, std::array<std::array<uint64_t, NUM_CPUS>, champsim::to_underlying(access_type::NUM_TYPES)>{})
        , bank_misses(num_banks, std::array<std::array<uint64_t, NUM_CPUS>, champsim::to_underlying(access_type::NUM_TYPES)>{})
        , bank_avg_miss_latency(num_banks, 0.0)
        , bank_total_miss_latency(num_banks, 0) {}
};
