...
void print_bank_stats(std::ostream& stream, const std::string& name, const std::vector<std::array<std::array<uint64_t, NUM_CPUS>, champsim::to_underlying(access_type::NUM_TYPES)>>& bank_hits, const std::vector<std::array<std::array<uint64_t, NUM_CPUS>, champsim::to_underlying(access_type::NUM_TYPES)>>& bank_misses, const std::vector<double>& bank_avg_miss_latency)
{
  ...
  for (std::size_t bank = 0; bank < bank_hits.size(); ++bank) {
    fmt::print(stream, "\n{} Bank {} Statistics:\n", name, bank);
    for (std::size_t cpu = 0; cpu < NUM_CPUS; ++cpu) {
      uint64_t TOTAL_HIT = 0, TOTAL_MISS = 0;
      for (const auto& type : types) {
        TOTAL_HIT += bank_hits[bank][type.second][cpu];
        TOTAL_MISS += bank_misses[bank][type.second][cpu];
      }

      fmt::print(stream, "  CPU {} TOTAL     ACCESS: {:10d} HIT: {:10d} MISS: {:10d}\n", cpu, TOTAL_HIT + TOTAL_MISS, TOTAL_HIT, TOTAL_MISS);
      for (const auto& type : types) {
        fmt::print(stream, "  CPU {} {:<12s} ACCESS: {:10d} HIT: {:10d} MISS: {:10d}\n", cpu, type.first,
                   bank_hits[bank][type.second][cpu] + bank_misses[bank][type.second][cpu], bank_hits[bank][type.second][cpu], bank_misses[bank][type.second][cpu]);
      }
    }
    fmt::print(stream, "  AVERAGE MISS LATENCY: {:.4g} cycles\n \n", bank_avg_miss_latency[bank]);
  }
}
...
void champsim::plain_printer::print(CACHE::stats_type stats)
{
  ...
  //Bank stats printing
    print_bank_stats(stream, stats.name, stats.bank_hits, stats.bank_misses, stats.bank_avg_miss_latency);
}
